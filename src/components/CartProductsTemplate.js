...
class CartProducts extends React.Component {
  ...
  moveImage = (toTheRight) => {
    ...
    if (toTheRight) {
      index =
        selectedImage < this.props.gallery.length - 1 ? selectedImage + 1 : 0;
    } else {
      index =
        selectedImage === 0 ? this.props.gallery.length - 1 : selectedImage - 1;
    }
        ...
        <div className="flex-wrapper">
          <div className="item-details">
            ...
            <h3 className="item-amount">
              {currency}
              {Number.parseFloat(
                getAmount(product.productDetails.prices, currency)
              ).toFixed(2)}
            </h3>
            {product.productDetails.attributes.map((attribute, index) => {
              const selectedAttribute =
                product.productDetails.selectedAttributes.find(
          ...
          <div className="right-items">
            <div className="product-quantity">
              <img
                onClick={() => this.updateProductQuantity(index, "add")}
                src={plusIcon}
                alt="increase-quantity"
              />
              <h4 className="quantity">{product.quantity}</h4>
              <img
                onClick={() => this.updateProductQuantity(index, "subtract")}
                src={minusIcon}
                alt="reduce-quantity"
              />
            </div>
